...
function getScoreTier(score) {
  if (score <= 300)
    return { label: "CARBON ROOKIE", className: "score-red", color: "#ff4d4d" };
  if (score <= 500)
    return { label: "ECO AWARE", className: "score-orange", color: "#f39c12" };
  if (score <= 700)
    return {
      label: "GREEN CHAMPION",
      className: "score-yellowgreen",
      color: "#b9ff36",
    };
  return {
    label: "CARBON HERO",
    className: "score-brightgreen",
    color: "#39ff14",
  };
}
...
function getCityTip(city, totalTonnes) {
  const tips = {
    Bengaluru:
      "Bengaluru households usually get strong impact from efficient cooling and shorter commute swaps like metro + bus.",
    Mumbai:
      "In Mumbai, rail-based commuting is a big lever. Reducing taxi or car dependence usually moves the needle fast.",
    Delhi:
      "Delhi users often benefit most from cleaner transport choices and summer electricity efficiency.",
    Chennai:
      "In Chennai, cooling demand matters a lot, so fans, efficient AC settings, and solar can pay off quickly.",
    Hyderabad:
      "Hyderabad footprints often improve fastest through electricity efficiency and shorter private-vehicle trips.",
    Kolkata:
      "Kolkata already has strong public transport options, so diet and home energy can become the next big win.",
    Pune: "Pune users can usually gain fast by trimming petrol travel and improving apartment electricity efficiency.",
    Ahmedabad:
      "Ahmedabad users often benefit from cooling efficiency and reducing high-temperature daytime car use.",
    Jaipur:
      "Jaipur households can often lower emissions with efficient cooling, insulation habits, and smarter LPG use.",
    Surat:
      "In Surat, efficient appliances and shifting frequent road trips can meaningfully lower annual emissions.",
  };
  const benchmark = getCityAverage(city);
  const compare =
    totalTonnes <= benchmark
      ? "You are already beating your city benchmark."
      : "You are currently above your city benchmark.";
...
function getHotspotInsight(breakdown) {
  const categories = [
    {
      key: "electricity",
      label: "Electricity",
      annual: breakdown.annualKg.electricity,
      savingsTip: (val) =>
        `Switching to solar could save ${((val * 0.8) / 1000).toFixed(1)} tonnes/yr`,
    },
    {
      key: "petrol",
      label: "Petrol vehicle",
      annual: breakdown.annualKg.petrol,
      savingsTip: (val) =>
        `Switching to an EV saves ${((val * 0.5) / 1000).toFixed(1)} tonnes/yr`,
    },
    {
      key: "train",
      label: "Train travel",
      annual: breakdown.annualKg.train,
      savingsTip: (val) =>
        `Reducing trips could save ${((val * 0.2) / 1000).toFixed(1)} tonnes/yr`,
    },
    {
      key: "lpg",
      label: "LPG cooking",
      annual: breakdown.annualKg.lpg,
      savingsTip: (val) =>
        `Switching to induction cooking saves ${((val * 0.4) / 1000).toFixed(1)} tonnes/yr`,
    },
    {
      key: "diet",
      label: "Diet",
      annual: breakdown.annualKg.diet,
      savingsTip: (val) =>
        `Shifting to a plant-based diet saves ${((val * 0.3) / 1000).toFixed(1)} tonnes/yr`,
    },
  ];
  ...
  const pct =
    breakdown.totalAnnualKg === 0
      ? 0
      : (top.annual / breakdown.totalAnnualKg) * 100;
...
function calculateBreakdown(inputs) {
  const stateEF =
    inputs.state && inputs.state !== "National" && EF.electricity[inputs.state]
      ? EF.electricity[inputs.state]
  ...
  const totalAnnualKg = Object.values(annualKg).reduce(
    (sum, value) => sum + value,
    0,
  );
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    if (dietSelect) {
      dietSelect.addEventListener("change", (e) => {
        if (e.target.value === "omnivore") {
          if (window.showToast)
            window.showToast(
              "Nudge: Shifting to a vegetarian diet could save ~100kg CO2 per year!",
              "leaf",
            );
        } else if (e.target.value === "vegan") {
          if (window.showToast)
            window.showToast(
              "Awesome! A vegan diet has the lowest carbon footprint.",
              "heart",
            );
        }
      });
    ...
    form.addEventListener("submit", (event) => {
      ...
      baseFootprint = {
        ...
        fullData: {
          scope1:
            breakdown.annualKg.petrol / 1000 + breakdown.annualKg.lpg / 1000,
          scope2: breakdown.annualKg.electricity / 1000,
          scope3:
            breakdown.annualKg.train / 1000 + breakdown.annualKg.diet / 1000,
          breakdown,
        },
      };
      ...
      const originalBtnHtml = calcBtn
        ? calcBtn.innerHTML
        : "Calculate Carbon Footprint";
      if (calcBtn) {
        calcBtn.innerHTML =
          '<i data-lucide="loader-2" class="spin"></i> Fetching AI Insights...';
        refreshIcons();
      }
    ...
    document.querySelectorAll(".chart-download-btn").forEach((btn) => {
      btn.addEventListener("click", () =>
        downloadChartImage(btn.dataset.chart),
      );
    });
    ...
    [evSlider, solarSlider].forEach((slider) => {
      slider.addEventListener("input", () => {
        ...
        document.getElementById("solar-val").innerText =
          `${solarSlider.value}%`;

        const petrolPortion =
          (baseFootprint.breakdown?.annualKg.petrol || 0) / 1000;
        const elecPortion =
          (baseFootprint.breakdown?.annualKg.electricity || 0) / 1000;
        const evSavings = petrolPortion * evPct * 0.5;
        const solarSavings = elecPortion * solarPct;
        const newTotal = Math.max(
          0,
          baseFootprint.total - evSavings - solarSavings,
        );

        document.getElementById("projected-total").innerText =
          newTotal.toFixed(2);
      });
    });
    ...
    function editPreviousInputs() {
      ...
      if (document.getElementById("state"))
        document.getElementById("state").value =
          baseFootprint.inputs.state || "National";
      document.getElementById("electricity").value =
        baseFootprint.inputs.electricity;
    ...
    function updateInsightCards(data) {
      document.getElementById("city-insights").innerText = getCityTip(
        data.city,
        data.total,
      );
      document.getElementById("hotspot-insight").innerText = getHotspotInsight(
        data.breakdown,
      );
    }
    ...
    function renderHistorySection(history) {
      ...
      if (history.length < 2) {
        trendEl.innerText =
          "Complete one more calculation to unlock your improvement trend.";
      ...
      const changePct =
        previous === 0 ? 0 : Math.abs(((latest - previous) / previous) * 100);
      ...
      const rows = [
        {
          ...
          monthly: breakdown.monthlyKg.electricity,
          annual: breakdown.annualKg.electricity,
        },
        {
          label: "Petrol Vehicle (Scope 1)",
          monthly: breakdown.monthlyKg.petrol,
          annual: breakdown.annualKg.petrol,
        },
        {
          label: "Train Travel (Scope 3)",
          monthly: breakdown.monthlyKg.train,
          annual: breakdown.annualKg.train,
        },
        {
          label: "LPG Cooking (Scope 1)",
          monthly: breakdown.monthlyKg.lpg,
          annual: breakdown.annualKg.lpg,
        },
        {
          label: "Diet (Scope 3)",
          monthly: breakdown.monthlyKg.diet,
          annual: breakdown.annualKg.diet,
        },
      ];
      const totalMonthly = Object.values(breakdown.monthlyKg).reduce(
        (sum, value) => sum + value,
        0,
      );
      ...
      tbody.innerHTML =
        rows
          .map((row) => {
            ...
            const rowClass =
              pct > 30 ? "table-high" : pct >= 15 ? "table-mid" : "table-low";
          ...
          .join("") +
        `<tr class="table-total"><th scope="row">TOTAL</th><td>${totalMonthly.toFixed(1)}</td><td>${breakdown.totalAnnualKg.toFixed(1)}</td><td>100%</td></tr>`;
    }
    ...
    function generateShareCard(data) {
      ...
      ctx.fillText(
        `Your footprint: ${data.total.toFixed(2)} tonnes / year`,
        170,
        660,
      );
      ctx.fillText(
        `India average: ${INDIA_AVG.toFixed(1)} tonnes / year`,
        170,
        735,
      );
      ctx.fillText(
        `City benchmark: ${data.city} ${getCityAverage(data.city).toFixed(2)} tonnes`,
        170,
        810,
      );
    ...
    function exportHistoryCsv() {
      const history = baseFootprint.history?.length
        ? baseFootprint.history
        : getFootprintHistory();
      ...
      const lines = [
        ...
        ...history.map(
          (entry) =>
            `${entry.timestamp},${entry.total},${entry.score},${entry.city}`,
        ),
      ];
...
if (typeof window !== "undefined") {
  window.toggleChallenge = function (element, id) {
    ...
    document.getElementById("challenges-count").innerText =
      `${activeChallenges}/3`;
  };
  ...
  window.toggleAIChat = function () {
    chatOpen = !chatOpen;
    document.getElementById("ai-body").style.display = chatOpen
      ? "flex"
      : "none";
  ...
  function getRuleBasedResponse(msg, fp) {
    ...
    if (!isCalculated && msg.match(/reduce|score|footprint|my data|tips/))
      return "Please complete the calculator on the main screen first! I need your data to give you personalized advice.";

    if (
      msg.match(
        /reduce.*score|improve.*score|lower.*footprint|tips|give me personalized tips/,
      )
    ) {
      let highest = "Scope 1 (Direct Fuel)";
      let value = fp.scope1;
      if (fp.scope2 > value) {
        highest = "Scope 2 (Electricity)";
        value = fp.scope2;
      }
      if (fp.scope3 > value) {
        highest = "Scope 3 (Indirect/Diet)";
        value = fp.scope3;
      }
      ...
      if (highest.includes("Scope 1")) {
        ...
        const sEF =
          fp.inputs.state && EF.electricity[fp.inputs.state]
            ? EF.electricity[fp.inputs.state]
            : EF.electricity.National;
    ...
    if (msg.match(/average|compare|better|worse/)) {
      ...
      if (fp.total < INDIA_AVG)
        return `Yes! You are at ${fp.total.toFixed(2)} tonnes, which is ${Math.abs(diff)}% better than the Indian average of 1.9 tonnes.`;
      return `Currently, you are at ${fp.total.toFixed(2)} tonnes, which is ${diff}% higher than the Indian average of 1.9 tonnes.`;
    }

    if (msg.match(/what is scope 1|scope 1/))
      return "Scope 1 covers direct emissions like petrol in your car or LPG used for cooking.";
    if (msg.match(/what is scope 2|scope 2/))
      return "Scope 2 covers indirect emissions from purchased electricity.";
    if (msg.match(/what is scope 3|scope 3/))
      return "Scope 3 covers other indirect emissions like food and public transport.";
  ...
  function loadSavedInputs() {
    try {
      ...
      if (saved) {
        ...
        if (document.getElementById("state"))
          document.getElementById("state").value = inputs.state || "National";
  ...
  function initOnboardingTour() {
    ...
    const steps = [
      {
        el: document.querySelector(".gauge-container"),
        msg: "Your AI-calculated Carbon Credit Score. Aim for 600+!",
      },
      {
        el: document.getElementById("equivalent-facts"),
        msg: "See your footprint in real-world terms 🌍",
      },
      {
        el: document.querySelector(".scope-labels").parentNode,
        msg: "Understand WHERE your emissions come from",
      },
      {
        el: document.getElementById("leaderboardChart").parentNode.parentNode,
        msg: "Compare yourself against 8 Indian cities",
      },
      {
        el: document.querySelector(".share-actions"),
        msg: "Challenge friends and unlock the Influencer badge!",
      },
    ];
  ...
  window.addDailyLog = function () {
    ...
    if (typeEl.value === "electricity") {
      const sEF =
        window.baseFootprint &&
        window.baseFootprint.inputs &&
        window.baseFootprint.inputs.state &&
    ...
    if (window.baseFootprint && window.baseFootprint.total > 0) {
      ...
      if (typeEl.value === "electricity")
        window.baseFootprint.scope2 += addedTonnes;
      ...
      window.baseFootprint.score = window.calculateCreditScore(
        window.baseFootprint.total,
      );
      ...
      document.getElementById("total-co2").innerText =
        window.baseFootprint.total.toFixed(2);
      document.getElementById("projected-total").innerText =
        window.baseFootprint.total.toFixed(2);
      document.getElementById("credit-score-val").innerText =
        window.baseFootprint.score;
      ...
      window.showToast(
        `Logged ${val} ${typeEl.value}. Added ${addedKg.toFixed(2)} kg CO2 to footprint.`,
        "check-circle",
      );
    } else {
  ...
  window.shareToWhatsApp = function () {
    ...
    const text = encodeURIComponent(
      `I just checked my Carbon Credit Score on EcoTrack India! I scored ${window.baseFootprint.score} / 850. Check yours out here: `,
    );
  ...
  window.shareToTwitter = function () {
    const url = encodeURIComponent(window.location.href);
    const text = encodeURIComponent(
      `I just scored ${window.baseFootprint.score}/850 on the EcoTrack India Carbon Calculator! 🌱 Can you beat my score? `,
    );
    window.open(
